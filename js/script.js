...
const controls=document.getElementById("galleryControls"),galleryImage=document.getElementById("galleryImage"),galleryCaption=document.getElementById("galleryCaption");if(controls&&galleryImage&&galleryCaption){galleryItems.forEach((item,index)=>{const btn=document.createElement("button");btn.textContent = item[2].replace(/^DS\d+_[A-Z0-9]+\s*/, "");if(index===5)btn.classList.add("active");btn.addEventListener("click",()=>{document.querySelectorAll(".gallery-controls button").forEach(b=>b.classList.remove("active"));btn.classList.add("active");galleryImage.src=item[1];galleryCaption.textContent=item[2]});controls.appendChild(btn)})}
